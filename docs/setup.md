...
Set `GOOGLE_SHEET_ID`, `WIF_PROVIDER`, and `WIF_SERVICE_ACCOUNT` as private runtime secrets; use `SHEETS_AUTH_MODE=wif`. For local verification, supply Google Application Default Credentials with access to that Sheet, or deliberately select the `GOOGLE_SERVICE_ACCOUNT_JSON_B64` fallback. A fallback key remains private and must be rotated; do not keep both a fallback and keyless credential mode active ambiguously.
...
Configure private runtime secrets with `scripts/configure_github.sh --repo OWNER/PRIVATE_RUNTIME_REPO --plan`, then `--apply` after reviewing the plan. Replace both runtime-template SHA placeholders with the same reviewed public commit. Validate an idempotent real scan and identical rerun before setting `TRACKER_ENABLED=true`. See [operations](operations.md) for schedules and [cost controls](cost-controls.md) for billing gates.
